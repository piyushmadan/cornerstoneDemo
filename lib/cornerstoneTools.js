...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
          ...
          if($("#presenter").is(":checked") && color == cornerstoneTools.toolColors.getToolColor()){
                    console.log("drawLine",data, color, lineWidth);
...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            context.strokeStyle = color;
            context.lineWidth = lineWidth;
            cornerstoneTools.drawEllipse(context, leftCanvas, topCanvas, widthCanvas, heightCanvas,color);
            context.closePath();
...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
          ...
          if($("#presenter").is(":checked") && !data.active){
                    console.log("drawLine",data, color, lineWidth);
                    socket.emit('drawLine',{
                        data: data,
                        color: color,
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            //TODO: Enter socket.io  for PROBE HERE
...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            // add this condition after test data.active TRUE 
          if($("#presenter").is(":checked") && !data.active){
                    console.log("drawRect",leftCanvas, topCanvas, widthCanvas, heightCanvas);
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function defaultStrategy(eventData) {
      ...
      if($("#presenter").is(":checked") ){
                console.log("wwwc",eventData.viewport.voi);

                socket.emit('wwwc',{
                    webSocketId: socket.webSocketId,
                    eventType: 'wwwc',
                    voi: eventData.viewport.voi
                });
        }
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function defaultStrategy(eventData, ticks) {
        ...
        cornerstone.setViewport(element, viewport);
           ...
           // Your logic here
//          if($("#presenter").is(":checked") && new Date().getTime() - localStorage.getItem('lastZoomSentTime')  > 400 ){
          if($("#presenter").is(":checked") ){
                ...
                localStorage.setItem('lastZoomSentTime', new Date().getTime());
...
(function(cornerstone, cornerstoneTools) {
    ...
    function drawHandles(context, renderData, handles, color, fill) {
        ...
        Object.keys(handles).forEach(function(name) {
          ...
          if($("#presenter").is(":checked") && color== cornerstoneTools.toolColors.getToolColor()){
                  console.log("drawHandles",context, renderData, handles, color, fill);
                  console.log("****");
                  console.log(handles);
                  console.log(renderData);
                  console.log("****");

                    socket.emit('drawHandles',{
                        renderData: new Object(),
                        handles:handles,
                        color: color,
                        fill: fill,
                        webSocketId: socket.webSocketId,
                        eventType: 'drawHandles'
                    });
            }
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function renderActiveReferenceLine(context, eventData, targetElement, referenceElement) {
        ...
        console.log("in renderActiveReferenceLine");
        console.log(arguments);
...
(function(cornerstoneTools) {
    ...
    function drawEllipse(context, x, y, w, h,color) {
        var kappa = 0.5522848,
        ...
        // g_context2 = new Object();
        // g_context2 = context;

        console.log(context);
        console.log(cornerstoneTools.toolColors.getToolColor());
        console.log(context.fillStyle);

      if($("#presenter").is(":checked") && color == cornerstoneTools.toolColors.getToolColor()){
//      if($("#presenter").is(":checked")){

          var viewport = cornerstone.getViewport(context.canvas.parentElement);

        console.log("drawEllipse",context, x, y, w, h);

          var enabledElement = cornerstone.getEnabledElement(context.canvas.parentElement);
          var t = cornerstone.internal.getTransform(enabledElement)
          var transformedPoints =  t.transformPointInverse(x,y)


                socket.emit('drawEllipse',{
                    x: transformedPoints.x ,
                    y: transformedPoints.y ,
                    w: w,
                    h: h,
                    webSocketId: socket.webSocketId,
                    eventType: 'drawEllipse'
...
(function(cornerstoneTools) {
    ...
    function drawTextBox(context, text, x, y, color) {
        ...
        // Too many socket requests.. so sending only if color is active
...
//      if($("#presenter").is(":checked") ){
          ...
          var transformedPoints =  t.transformPointInverse(x,y)
                ...
                console.log("drawTextBox",context,text,x,y,color);

                socket.emit('drawTextBox',{
                    text:text,
                    x: transformedPoints.x,
                    y: transformedPoints.y,
                    color: color,
                    webSocketId: socket.webSocketId,
                    eventType: 'drawTextBox'
                });
        }
